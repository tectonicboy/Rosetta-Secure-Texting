...
int main(){
    ...
    char    username[SMALL_FIELD_LEN]     = "Levin";
    ...
    printf("[OK]  RTF Simulation 1: Levin is calling login() now.\n");
    status = login((unsigned char*)pwd, pwd_len, full_save_dir);
    if(status){
        printf("[ERR] RTF Simulation 1: Levin could not login.\n");
        exit(1);
    }
    else{
        printf("[OK]  RTF Simulation 1: Levin logged in!\n");
    }
    status = join_chatroom((unsigned char*)room_name, room_name_len,
                           (unsigned char*)username,  username_len);
    if(status){
        printf("[ERR] RTF Simulation 1: Levin could not join a chat room.\n");
        if(status == 2){
            printf("[ERR] RTF Simulation 1: Server reply to Levin's JOIN_ROOM\n"
                   "                        request took too long.\n");
        }
        exit(1);
    }
    else{
        printf("[OK]  RTF Simulation 1: Levin joined a chat room!\n");
    }
    sleep(6);
    /* Send messages */
