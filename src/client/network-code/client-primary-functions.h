...
void* begin_polling(__attribute__((unused)) void* input)
{
    ...
    for(;;){
        ...
        /* Handle here replies for join_room and create_room commands. */
        if( *reply_type_ptr == PACKET_ID_10 ){
            ...
            /* Here is one of 2 possible places where GUI renders the graphics
...
/* PAYLOAD DIAGRAM: Server replied to our poll request with pending messages.
...
u8 reg(u8* password, int password_len, char* save_dir)
{
    ...
    /* Prepare a buffer containing all the stuff to be saved. */
...
/* PAYLOAD DIAGRAM: Client sends their encrypted long-term public key to server.
                 ...
                 unsigned char* userid, int userid_len)
{
    ...
    /* IF bad msg_20 construction, abort. */
    if(status){
...
uint8_t send_text(unsigned char* text, uint64_t text_len)
{
     ...
     * others will detect our new message. This is to avoid the discrepancy
     * in latency from locally displaying the user's message right away, to
     * waiting for the server to send it to us, and missing an already sent
     * text message by somebody else in the chatroom while that was happening.
     */
