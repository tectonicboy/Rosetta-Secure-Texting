...
def generate_and_add_large_ints(initial_kick, count, nr_operand_bits):
    ...
    for i in range(count):
        operands_1[i] = initial_kick + (17_000_000_000_000_000_000 * pow(i, 90))
        operands_2[i] = initial_kick + (12_111_222_333_444_555_666 * pow(i, 90))
...
start_bigint   = 0xFF00000000000000
multiplier     = 0xFF00000000000000
operand_bits   = 64
nr_add_results = 10000000

# Will make 50 * 64 ~= 3200-bit starting BigInt.
for _ in range(50):
